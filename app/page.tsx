...
import { STACK_LAYERS, PULSE_NEWS } from "@/lib/data";
...
type DirectoryLayer = {
  ...
  name: string;
  rank: number;
  icon?: ComponentType<{ size?: number; strokeWidth?: number; className?: string }> | null;
  color?: string | null;
};
...
function EntityCard({
  ...
  const fallbackLayerInfo = STACK_LAYERS.find((l) => l.slug === layerSlug) || null;
  const gradient =
    layerInfo?.color || fallbackLayerInfo?.color || "from-gray-500 to-gray-400";
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-4 mb-5">
          ...
          >
            {entity.logo_url ? (
              ...
              <span className="font-black">{entity.name.charAt(0).toUpperCase()}</span>
            )}
          </div>
        ...
        <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
          <span className="text-[9px] font-black text-white/20 uppercase tracking-[0.35em]">
            {layerInfo?.name || fallbackLayerInfo?.name || "Entity"}
          </span>
          {entity.website_url ? (
...
function DirectoryContent() {
  ...
  const [layers, setLayers] = useState<DirectoryLayer[]>(() =>
    STACK_LAYERS.map((l, index) => ({
      id: String(l.id),
      slug: l.slug,
      name: l.name,
      rank: index + 1,
  ...
  const [pulseItems, setPulseItems] = useState<PulseRailItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchLayers() {
      ...
      try {
        ...
        const rows = (json.data || json.layers || []) as ApiLayerRow[];
        const nextLayers: DirectoryLayer[] = rows
          .map((l) => ({
            id: String(l.id),
            slug: l.slug,
            name: l.name,
            rank: typeof l.rank === "number" ? l.rank : Number.MAX_SAFE_INTEGER,
            icon: getIconByName(l.icon_name || null),
            color: l.color_gradient || null,
          }))
          .filter((l) => l.slug && l.name)
          .sort((a, b) => a.rank - b.rank);

        if (!done && nextLayers.length > 0) {
          setLayers(nextLayers);
        }
      } catch {
        // Keep fallback layers
      } finally {
  ...
  useEffect(() => {
    ...
    async function fetchEntities() {
      ...
      try {
        ...
        const nextEntities = (data.entities || data.data || []) as DirectoryEntity[];
    ...
    async function fetchPulse() {
      ...
      try {
        ...
        const { data, error } = await supabase
          .from("pulse_updates")
          .select(
            "id, title, type, created_at, source_name, url, related_layer_id, related_entity_id",
          )
          .order("created_at", { ascending: false })
          .limit(5);

        if (error) throw error;
        type DbPulseUpdateRow = {
          id: string;
          title: string;
          type: string | null;
        ...
        const rows = (data || []) as unknown as DbPulseUpdateRow[];
        const items: PulseRailItem[] = rows.map((p) => ({
          id: p.id,
          title: p.title,
          type: String(p.type || "update"),
          time: formatRelativeTime(p.created_at) || "just now",
          source: p.source_name || null,
          url: p.url || null,
        }));
        if (!done) setPulseItems(items);
      } catch {
        const fallback: PulseRailItem[] = PULSE_NEWS.slice(0, 5).map((n) => ({
          id: n.id,
          title: n.title,
          type: n.type,
          time: n.time,
        ...
        if (!done) setPulseItems(fallback);
      } finally {
        if (!done) setPulseLoading(false);
      }
    }
  ...
  const layerEntityIds = useMemo(() => {
    return new Set(entities.map((e) => String(e.id)));
  }, [entities]);
  ...
  }, [activeLayer, entityById, layerEntityIds, search, stacks]);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-9 lg:divide-x lg:divide-white/10">
            ...
            <aside className="lg:col-span-3 py-10 lg:py-14 lg:px-8">
              <div className="lg:sticky lg:top-32">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-[10px] font-black uppercase tracking-[0.5em] text-white/40 flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" />
                    Real-Time Pulse
                  </h2>
                  <Link href="/pulse" className="text-white/25 hover:text-white transition-colors">
                  ...
                  <div className="space-y-px bg-white/10 border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
                    {pulseItems.map((item) => (
                      <Link
                        key={String(item.id)}
                        href="/pulse"
                        className="block bg-[#050507] p-5 hover:bg-white/[0.02] transition-colors"
                      >
                        ...
                        <div className="text-sm font-black text-white uppercase tracking-tighter leading-tight line-clamp-2">
                          {item.title}
                        </div>
                        {item.source ? (
                          <div className="mt-2 text-[10px] text-white/25 font-bold uppercase tracking-widest">
                            Source: {item.source}
                          </div>
                        ) : null}
                      </Link>
                    ))}
                    <div className="bg-[#050507] p-5 border-t border-white/10">
                      <Link
                        href="/pulse"
                        className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40 hover:text-white transition-colors flex items-center justify-between"
                      >
                        <span>View Full Pulse</span>
                        <ArrowUpRight size={14} />
                      </Link>
                    </div>
                  </div>
